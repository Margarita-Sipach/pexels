...
export const getPhotos = async (page = 1, lang = 'en') => {
  const res = await axios.get(
    `${API}curated?&locale=${lang === 'en' ? 'en-US' : 'ru-RU'}&page=${page}`,
    {
...
export const getPhotosById = async (
  ...
  const res = await axios.get(
    `${API}search?query=${id}&locale=${
      lang === 'en' ? 'en-US' : 'ru-RU'
    }&page=${page}${Object.entries(params).reduce(
